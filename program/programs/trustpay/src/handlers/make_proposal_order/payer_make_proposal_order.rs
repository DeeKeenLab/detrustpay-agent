...
pub fn process_payer_make_proposal_order(
    ...
    order.payer_made_proposal_amount = counter_amount;
    order.payer_made_proposal_expiry = proposal_expiry;
    order.version = order
        .version
        .checked_add(1)
    ...
    order.payer_made_proposal_date = clock.unix_timestamp;
    ...
    if let Some(message_value) = message {
        ...
        order.payer_message = message_value;
        order.payer_message_is_encrypted = is_encrypted.unwrap_or(false);
        order.payer_message_nonce = nonce.unwrap_or([0u8; 12]);
        order.payer_message_date = clock.unix_timestamp;
        let message_ephemeral = ephemeral_pubkey.unwrap_or_default();
        if message_ephemeral != Pubkey::default() {
            order.payer_ephemeral_pubkey = message_ephemeral;
        }
    }
