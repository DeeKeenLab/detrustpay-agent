...
pub fn process_payer_make_proposal_order(
    ...
    ctx.accounts.order_account.payer_made_proposal_amount = counter_amount;
    ctx.accounts.order_account.payer_made_proposal_expiry = proposal_expiry;
    ...
    ctx.accounts.order_account.payer_made_proposal_date = clock.unix_timestamp;
    ...
    if let Some(message_value) = message {
        ...
        ctx.accounts.order_account.payer_message = message_value;
        ctx.accounts.order_account.payer_message_is_encrypted = is_encrypted.unwrap_or(false);
        ctx.accounts.order_account.payer_message_nonce = nonce.unwrap_or([0u8; 12]);
        ctx.accounts.order_account.payer_message_date = clock.unix_timestamp;
        let message_ephemeral = ephemeral_pubkey.unwrap_or_default();
        if message_ephemeral != Pubkey::default() {
            ctx.accounts.order_account.payer_ephemeral_pubkey = message_ephemeral;
        }
    }
...
pub struct PayerMakeProposalOrder<'info> {
    #[account(mut)]
