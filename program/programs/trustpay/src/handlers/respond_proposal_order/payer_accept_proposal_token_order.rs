...
use super::{apply_payer_response, ensure_payer_can_respond};

pub fn process_payer_accept_proposal_token_order(
    ...
    ensure_payer_can_respond(order, &id)?;
    require!(
    ...
    let counter_amount = order.payee_made_proposal_amount;
    let payment_key = ctx.accounts.buyer_order_account.key();
    let discount = settlement_fee_discount(SettlementFeeDiscountMode::PayerAcceptPayeeProposal);
    ...
    let clock = Clock::get()?;
    ctx.accounts.buyer_order_account.order.closed = true;
    ctx.accounts.buyer_order_account.order.closed_date = clock.unix_timestamp;
    ctx.accounts.buyer_order_account.order.closed_reason = OrderClosedReason::PayerAcceptProposal;
    sync_order_pair(
...
#[derive(Accounts)]
pub struct PayerAcceptProposalTokenOrder<'info> {
    #[account(mut)]
